...
async fn background_main() {
    ...
    while let Some(_change) = changes.next().await {
        let (should_exit, should_signal) = {
            let (current_id, current_cmd, current_pi_resp, exit) = doc_handle.with_document(|doc| {
            ...
            let signal = exit
                ...
                || (current_pi_resp.is_some() && current_pi_resp != last_pi_response);
            ...
            if current_pi_resp != last_pi_response {
                last_pi_response = current_pi_resp;
            }
